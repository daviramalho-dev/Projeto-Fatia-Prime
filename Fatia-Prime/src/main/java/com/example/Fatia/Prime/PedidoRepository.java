...
public interface PedidoRepository extends JpaRepository<Pedido, Long> {
	...
	@Query("select distinct p from Pedido p join fetch p.usuario u left join fetch p.itens i left join fetch i.produto where p.codigo = :codigo")
	Optional<Pedido> findByCodigoForConsulta(@Param("codigo") String codigo);

	@Query("select distinct p from Pedido p join fetch p.usuario u left join fetch p.itens i left join fetch i.produto "
		+ "where function('regexp_replace', coalesce(u.telefone, ''), '[^0-9]', '', 'g') = :telefone "
